...
export async function remove(path: string, options?: RemoveOptions) {
  ...
  const parsedOptions: ParsedOptions = {
    fileStandard: options?.fileStandard ?? fileStandards.forever,
    directoryStandard: options?.directoryStandard ?? directoryStandards.forever,
...
async function removeDirectory(dir: string, options: ParsedOptions) {
  ...
  const rejectedFiles = await getRejected(fileTasks, files);
